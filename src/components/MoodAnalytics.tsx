...
const MoodAnalytics = ({ moodEntries }: MoodAnalyticsProps) => {
  ...
  const getHeatmapColor = (avgMood: number) => {
    if (avgMood === 0) return 'bg-secondary';
    if (avgMood >= 4) return 'bg-green-500';
    if (avgMood >= 3) return 'bg-yellow-500';
    if (avgMood >= 2) return 'bg-orange-500';
    return 'bg-red-500';
  };

  const getHeatmapIntensity = (avgMood: number) => {
    if (avgMood === 0) return 'opacity-20';
    if (avgMood >= 4) return 'opacity-100';
    if (avgMood >= 3) return 'opacity-80';
    if (avgMood >= 2) return 'opacity-60';
    return 'opacity-80';
  };
              ...
              {heatmapData.map((item) => (
                <div key={item.day} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-10 rounded-lg relative overflow-hidden bg-secondary">
                    <div
                      className={`absolute inset-0 ${getHeatmapColor(item.avgMood)} ${getHeatmapIntensity(item.avgMood)} transition-all`}
                      style={{ width: `${(item.avgMood / 5) * 100}%` }}
                    />
                  </div>
