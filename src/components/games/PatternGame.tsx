    ...
    <div className="space-y-4">
      ...
      <div className="text-center py-2">
        ...
        <div className="flex items-center justify-center gap-2 flex-wrap">
          {pattern.map((s, i) => (
            <span key={i} className={`text-3xl ${s === '❓' ? 'animate-pulse' : ''}`}>{s}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {options.map((symbol, i) => (
          <button
            ...
            className="aspect-square rounded-lg bg-secondary hover:bg-secondary/80 text-2xl flex items-center justify-center transition-all hover:scale-105 active:scale-95"
          >
            {symbol}
          </button>
        ))}
