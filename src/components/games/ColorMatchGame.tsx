      ...
      <div className="grid grid-cols-2 gap-3">
        {options.map((color) => (
          <button
            ...
            className={`h-20 rounded-xl ${color.class} transition-all hover:scale-105 active:scale-95 ${
              feedback === 'correct' && color.name === targetColor.name ? 'ring-4 ring-green-300' : ''
            } ${feedback === 'wrong' && color.name === targetColor.name ? 'ring-4 ring-primary' : ''}`}
          />
        ))}
      </div>
    </div>
