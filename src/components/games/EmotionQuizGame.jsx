...
const EmotionQuizGame = () => {
    ...
    return (<div className="space-y-4">
      ...
      <div className="space-y-2">
        {shuffledOptions.map((option) => (<button key={option} onClick={() => handleAnswer(option)} className="w-full text-left px-4 py-3 rounded-lg bg-secondary hover:bg-secondary/80 text-sm text-foreground transition-all hover:scale-[1.01] active:scale-[0.99]">
            {option}
          </button>))}
      </div>
