      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <Card className="p-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {soundEnabled && (<div className="flex items-center gap-2 flex-1 min-w-[140px] max-w-[240px]">
                <span className="text-xs text-muted-foreground">Vol</span>
                <Slider value={[soundVolume * 100]} onValueChange={([v]) => setSoundVolume(v / 100)} max={100} step={5} className="flex-1"/>
              </div>)}
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {techniques.map(t => (<Card key={t.name} className={`cursor-pointer transition-all hover:scale-[1.02] ${selectedTechnique.name === t.name
                ? 'ring-2 ring-primary bg-primary/5'
                : 'hover:bg-secondary/50'}`} onClick={() => selectTechnique(t)}>
              ...
              <CardContent className="p-4 pt-0">
                ...
                <div className="flex gap-1 mt-2">
                  {t.steps.map((s, i) => (<span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-secondary text-secondary-foreground">
        ...
        <Card className="overflow-hidden">
          <CardContent className="p-8 flex flex-col items-center justify-center min-h-[400px]">
            ...
            <div className="relative flex items-center justify-center mb-8">
              {/* Outer glow ring */}
              <div className="absolute w-56 h-56 rounded-full opacity-20 transition-transform" style={{
            ...
            transitionTimingFunction: 'ease-in-out',
              ...
              {/* Main circle */}
              <div className="w-44 h-44 rounded-full flex items-center justify-center transition-transform border-4" style={{
            ...
            {/* Step indicators */}
            {isActive && (<div className="flex gap-2 mb-6">
                {selectedTechnique.steps.map((s, i) => (<div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === currentStep ? 'w-8 bg-primary' : 'w-4 bg-muted'}`}/>))}
              </div>)}

            {/* Controls */}
            <div className="flex gap-3">
              {!isActive && cycles === 0 && (<Button onClick={start} size="lg">
                  <Play className="w-4 h-4 mr-2"/> Start
                </Button>)}
              {isActive && (<Button onClick={togglePause} variant="outline" size="lg">
                  <Pause className="w-4 h-4 mr-2"/> Pause
                </Button>)}
              {!isActive && cycles > 0 && (<Button onClick={start} size="lg">
                  <Play className="w-4 h-4 mr-2"/> Resume
                </Button>)}
              {(isActive || cycles > 0) && (<Button onClick={reset} variant="secondary" size="lg">
                  <RotateCcw className="w-4 h-4 mr-2"/> Reset
                </Button>)}
            </div>

            {cycles > 0 && (<p className="text-sm text-muted-foreground mt-4">
