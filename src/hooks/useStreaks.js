...
export const useStreaks = () => {
    ...
    const awardIfEarned = async (name) => {
    ...
    const checkAndAwardAchievements = async (_userId, type, value) => {
        const candidates = [];
        if (type === 'mood_streak') {
            if (value >= 3)
                candidates.push('Getting Started');
            if (value >= 7)
                candidates.push('Week Warrior');
            if (value >= 30)
                candidates.push('Consistency King');
        }
        if (type === 'journal_streak') {
            if (value >= 3)
                candidates.push('Aspiring Writer');
            if (value >= 7)
                candidates.push('Consistent Writer');
        }
